...
| :--- | :--- |
| **Category** | Queue |
| **Type** | Traditional queue |
| **Primary Use Case** |  |
| **Strengths** | Thousands/sec |
| **Weaknesses** | Until consumed |
| **Best For** |  |
| **Never Use When** |  |
| **Max Scale** |  |
| **Consistency Model** |  |
...
1. **Broker-Centric State:** Tracks the exact status of every message. Messages are instantly deleted from memory/disk once explicitly ACKed by a consumer.
2. **Push Model:** Actively pushes messages to connected consumers, resulting in ultra-low sub-millisecond latency.
3. **Complex Routing:** Exchange bindings (Direct, Topic, Fanout, Headers) allow dynamic and highly sophisticated message routing rules.
4. **Memory Bound:** Designed for empty queues. If consumers back up, RabbitMQ starts paging to disk, degrading performance severely.
5. **Ephemeral by Nature:** Not designed to store historical data or allow message replay like a Kafka log.
6. **Queue Bottleneck:** A single queue lives on a single node (unless using Quorum Queues), limiting horizontal scale compared to Kafka partitions.
7. **Never Use When:** You need to stream millions of events per second, replay historical data, or require strict order processing across horizontal consumer clusters.
